# binance-data MCP Server Implementation Guide

You are working on implementing the binance-data MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: binance-data
- **Documentation**: [https://data-api.binance.vision](https://data-api.binance.vision)
- **Website**: [https://data-api.binance.vision](https://data-api.binance.vision)
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_binance_data",
      "get_binance_data_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["binance-data", "api", /* add relevant tags like "search", "data", etc. */]
}
```

### 2. Study the API Documentation

First, thoroughly review the API documentation at https://data-api.binance.vision to understand:
- Available endpoints
...
Add functions to call the binance-data API with retry support. Example pattern:
...
def call_binance_data_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call binance-data API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"binance-data API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_binance_data(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search binance-data for [specific data type].
    ...
    try:
        # The call_binance_data_api function already has retry logic
        results = call_binance_data_api(
...
```python
@mcp.tool()
async def get_binance_data_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_binance_data_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in binance-data.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_binance_data(query="example", limit=5)
   ```
...
### 9. Common binance-data Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on binance-data capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the binance-data documentation that you should implement:
...
- Check the binance-data API documentation: https://data-api.binance.vision
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make binance-data's capabilities easily accessible to AI agents through standardized MCP tools. 